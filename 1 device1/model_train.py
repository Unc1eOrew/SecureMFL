...
def train():
    ...
    # 保存密钥信息
    f = open('./key/public_key_n.txt', 'w')
    f.write(str(public_key.n))
    f = open('./key/private_key_p.txt', 'w')
    f.write(str(private_key.p))
    f = open('./key/private_key_q.txt', 'w')
    f.write(str(private_key.q))
    ...
    model = tf.keras.models.Sequential([
        ...
        tf.keras.layers.Dropout(0.3),  # 随机断开输入神经元的概率为 0.25，由于该层神经元过多，dropout正则化，防止过拟合
    ...
    print("agg test loss：%.4f" % test_score[0])
    print("agg test accuracy：%.4f" % test_score[1])
